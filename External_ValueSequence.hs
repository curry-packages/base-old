...
instance Curry (C_ValueSequence a) where
...
external_d_C_addVS :: a -> C_ValueSequence a
                   -> Cover -> ConstStore -> C_ValueSequence a
external_d_C_addVS x vs _ _ = Values (OP_Cons x (getValues vs))

external_d_C_failVS :: C_Int
                    -> Cover -> ConstStore -> C_ValueSequence a
external_d_C_failVS d@(C_Int d') (I# cd) _
  | isTrue# (d' <# cd) = FailVS d
  | otherwise          = Values (OP_List)

external_d_C_vsToList :: C_ValueSequence a -> Cover -> ConstStore -> OP_List a
external_d_C_vsToList (Values         xs) _  _  = xs
external_d_C_vsToList (FailVS  (C_Int d)) _  _  = failCons (I# d) defFailInfo
...
(|++|) :: Curry a => C_ValueSequence a -> C_ValueSequence a -> C_ValueSequence a
EmptyVS             |++| vs = vs
Values     xs       |++| vs = Values (d_OP_plus_plus xs (getValues vs)
...
failGreatest d (FailVS           d2) = FailVS (d_C_max d d2
